...
/**
 * goodix_device_write_pgm:
 *
 * Write a decoded frame as a plain PGM byte-for-byte like tool.py
 * write_pgm(): header "P2\n64 80\n4095\n", then rows of
 * GOODIX_SENSOR_WIDTH values.
 */
gboolean
goodix_device_write_pgm (const guint16 *img12,
                         const gchar   *path,
                         GError       **error)
{
  g_autoptr(GString) pgm = g_string_new (NULL);

  g_string_append_printf (pgm, "P2\n%d %d\n4095\n\n",
                          GOODIX_SENSOR_WIDTH, GOODIX_SENSOR_HEIGHT);

  for (gsize i = 0; i < GOODIX_SENSOR_PIXELS; i++)
    {
      if (i % GOODIX_SENSOR_WIDTH == 0)
        g_string_append_c (pgm, '\n');
      g_string_append_printf (pgm, "%4d ", img12[i]);
    }

  return g_file_set_contents (path, pgm->str, pgm->len, error);
}
